## Klotski Puzzle 2 (EASIER version)
## A sliding block puzzle where the player must move pieces to free the Fragment (F)
## Classic Klotski-style puzzle game - EASIER configuration
extends CanvasLayer
class_name KlotskiPuzzle2

signal puzzle_completed()

@export var puzzle_id: String = "klotski_2"  # Unique ID for tracking completion
...
@onready var time_label: Label = $PuzzlePanel/TimeLabel
@onready var skip_button: Button = $PuzzlePanel/SkipButton
...
func _ready() -> void:
	...
	if puzzle_timer:
		puzzle_timer.timeout.connect(_on_timer_timeout)
...
func _process(_delta: float) -> void:
	if _is_active and not _is_solved and not _showing_intro and puzzle_timer and time_label:
		var time_left = puzzle_timer.time_left
		var minutes = int(time_left / 60)
		var seconds = int(time_left) % 60
		time_label.text = "%02d:%02d" % [minutes, seconds]
		
		# Red color when running out of time
		if time_left < 30:
			time_label.modulate = Color(1.0, 0.2, 0.2)
			if int(time_left * 4) % 2 == 0: # Blinking effect
				time_label.modulate.a = 0.5
			else:
...
func _solve_puzzle(skipped: bool = false) -> void:
	_is_solved = true
	if puzzle_timer:
		puzzle_timer.stop()
	...
	if hint_label:
		...
		if skipped:
			hint_label.text = "PUZZLE SKIPPATO! (%d/%d)" % [collected, total]
		else:
...
func _show_intro() -> void:
	...
	text_label.text = "Un altro frammento! Questo sembra più semplice da liberare..."
...
func _on_timer_timeout() -> void:
	...
	if hint_label:
		hint_label.text = "Tempo scaduto! Puoi arrenderti se vuoi."
		hint_label.modulate = Color(1.0, 0.5, 0.5)

func _on_skip_pressed() -> void:
	# Award fragment and solve immediately
	_solve_puzzle(true)
