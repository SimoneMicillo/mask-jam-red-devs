## CursedObject
## Base script for objects that are only visible in Cursed World (when mask is ON).
## Usage: Attach to a Node3D and add it to the "cursed_only" group in the editor.
extends Node3D
class_name CursedObject
